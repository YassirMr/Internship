...
for i, nodei in node_index.items():
 ...
 opn = [SshJob(
	...
	required = settle_wireless_job,
        commands = [
            Run("iw dev atheros survey dump | grep noise | sed -n 1p >> /tmp/noise.txt") #The noise for the 2412 band, when using another frequency band, you should change the line picked by sed accordignly
        ]
	)
    for i,nodei in node_index.items() ]			#{k: node_index[k] for k in [i,j]}.items()
